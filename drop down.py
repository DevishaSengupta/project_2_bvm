...
class InventoryManagementSystem:
            ...
            if result:
                ...
                if current_status == 'Occupied':
                    ...
                    if self.execute_query(update_query, (item_id, subcom_place)):
                        ...
                        VALUES (%s, %s, %s, %s)
                        """
                        self.cursor.execute(trans_query, (item_id, subcom_place, 'added', datetime.now()))
    ...
    def execute_retrieve_product(self):
        ...
        item_id = product_mapping.get(selected_product)
        if not item_id:
        ...
        try:
            logging.info(f"Retrieving product details for {selected_product}")
...
class TransactionDialog:
    ...
    def ok_clicked(self):
        ...
        if item_id and quantity:
            ...
            messagebox.showerror("Error", "Please fill all required fields")def show_item_locations_dropdown(self):
    """Show dropdowns for each item displaying its occupied locations"""
    self.clear_dynamic_frame()
    ...
    item_names = ["bolt set", "gear", "bearing"]
    ...
    for item_name in item_names:
        ...
        query_id = "SELECT item_id FROM Items WHERE name = %s"
        result = self.execute_query(query_id, (item_name,))
        if not result:
            combo = ttk.Combobox(frame, values=["Item not found"], state="readonly", width=40)
            combo.set("Item not found")
